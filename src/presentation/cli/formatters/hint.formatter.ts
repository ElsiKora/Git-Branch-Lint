...
import { PatternMatchError, ProhibitedBranchError } from "../../../domain/errors/lint.error";
...
export class HintFormatter {
	 ...
	 */
	public format(error: unknown, config: IBranchLintConfig): string {
		let output: string = "";
	 ...
	 */
	private formatPatternMatchHint(config: IBranchLintConfig): string {
		let output: string = "";
	 ...
	 * @returns The formatted hint
	 */
	private formatProhibitedBranchHint(config: IBranchLintConfig): string {
